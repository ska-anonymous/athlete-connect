...
$sql = "SELECT * FROM athletes WHERE email='$user_login' OR login_id='$user_login'";
$qry = $pdo->prepare($sql);
$qry->execute();
...
if ($qry->rowCount()) {
    ...
    if (password_verify($user_pass, $password_hash)) {
        session_start();
        $_SESSION['athlete_logged'] = true;
        $_SESSION['athlete_id'] = $data['athlete_id'];
        $_SESSION['athlete'] = $data;
    } else {
        $response["error"] = true;
